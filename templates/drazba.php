...
if(isset($_POST['price']) && isset($_POST['discount_id'])) {
    if(!is_numeric($_POST['price']))$error = 'Cena musí byť celé čislo';
    $bidding_discount = Discount::getById($_POST['discount_id']);
    if(!isset($error) && !$bidding_discount->isActive())$error = 'Zľava nie je aktívna, refreshni stránku';
    if(!isset($error) && $_POST['price'] <= $bidding_discount->getLastBidPrice())$error = 'Cena musí byť vyššia ako posledná ponuka';
    if(!isset($error) && $_POST['price'] > Session::getLoggedUser()->getDisponsibleCredits())$error = 'Toľko kreditov nemáš';
    if(!isset($error)) {
...
<div class="discount rounded">
    ...
    <?php } else if(!Session::getLoggedUser()->hasEnoughCreditsToBid($discount)) {
